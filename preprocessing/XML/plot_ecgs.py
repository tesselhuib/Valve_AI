...
class ECGDrawing(dict):
    '''
    Takes a called `ECGXMLReader` instance and plots the lead-specific
    ...
    def __init__(self, update_keys: dict[str, str] | None = None) -> None:
        ...
        self.grid_linewidth = {"minor": 0.1, "major": 0.2}
        # the text constants
        self.text_pad_x = 40
    ...
    def __call__(self, ecgreader: Callable,
        ...
        """
        Creates an ECG drawing using either the waveforms `rhythm` or the
        median beats signals.

        Parameters
        ----------
        ecgreader : Callable
            An instance of the `ECGXMLReader`, or a similar, data class that
        ...
        >>> ecgxmlreader = ecgxmlreader(path=path, skip_empty=skip_empty,
        >>>                                verbose=verbose)
        >>> plt.ion()
        ...
        WARN2 = ('The Waveform unit attribute is not recognised: `{}`. Drawing assumes'
                 f' measurements are in {PDNames.MILLIVOLT}.')
        ...
        # #### get ecg data
        # confirm class has been called - result_dict should always be present
        ...
        if getattr(self, PDNames.WAVE_TYPE) == PDNames.WAVETYPE_RHYTHM:
            ...
            setattr(
                self,
                PDNames.PLOT_SAMPLING_NUMBER,
                getattr(getattr(self, PDNames.ECG_READER),
                        PDNames.RESULTS_DICT)[
                    PDNames.SAMPLING_NUMBER
                ],
            )
        else:
        ...
        if unit == PDNames.MICROVOLT:
            setattr(
                self,
                PDNames.ECG_SIGNAL,
                {k: v / 1000 for k, v in getattr(
                    self, PDNames.ECG_SIGNAL).items()},
            )
            pass
    ...
    def _set_canvas(self, auto_margins: bool = True,
        ...
        if ax is None:
            ...
            setattr(
                self, PDNames.PLOT_WIDTH, np.round(
                    width_n * fig_width * self.inch_mm)
            )
            setattr(
                self,
                PDNames.PLOT_HEIGHT,
            ...
            setattr(self, PDNames.PAPER_WIDTH, np.round(
                fig_width * self.inch_mm))
            setattr(self, PDNames.PAPER_HEIGHT, np.round(
                fig_height * self.inch_mm))
        # calculate marings
        if auto_margins is True:
            margin_left = 0.5 * (self.paper_w - getattr(
                self, PDNames.PLOT_WIDTH))
            # margin_right = .5 * (paper_w - width)
            margin_bottom = 10.0
            # Normalized in [0, 1]
            self.left = margin_left / self.paper_w
            self.right = self.left + getattr(
                self, PDNames.PLOT_WIDTH) / self.paper_w
    ...
    def _draw_signal(self, layout: list[list[str]],
        ...
        if hasattr(self, PDNames.PLOT_AXES) is False:
        ...
        # set some default
        if figsize is None:
            figsize = (self.paper_w / self.inch_mm,
                       self.paper_h / self.inch_mm)
        ...
        for numrow, row in enumerate(getattr(self, PDNames.PLOT_LAYOUT)):
            ...
            chunk_size = int(getattr(
                self, PDNames.PLOT_SAMPLING_NUMBER) / len(row))
            ...
            for numcol, k in enumerate(row):
                ...
                if getattr(self, PDNames.WAVE_TYPE) == PDNames.WAVETYPE_RHYTHM:
                    signal_temp = getattr(
                        self, PDNames.ECG_SIGNAL)[k][0:chunk_size]
                else:
                ...
                signal = v_delta + getattr(
                    self, PDNames.WAVE_SCALING) * signal_temp
                # update kwargs
                ...
                # update kwargs
                new_kwargs_text = _update_kwargs(
                    update_dict=kwargs_text,
                    zorder=3,
                    fontsize=8,
                )
                # plot the lead name
    ...
    def to_numpy(self, crop: bool = False, close: bool = True) -> np.ndarray:
        ...
        if hasattr(self, PDNames.PLOT_FIG) is False or\
                hasattr(self, PDNames.PLOT_UNIT) is False:
